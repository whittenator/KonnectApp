...
- (void) loadData
{
    ...
    if([aImageUrls count] > 0) {
        ...
        for (int i = 0; i < [aImageUrls count]; i++) {
            ...
            if([[aImageUrls objectAtIndex:i] isKindOfClass:[UIImage class]]) {
                // Set ImageView's Image directly
                  NSURL *url = [NSURL URLWithString:[[aImageUrls objectAtIndex:i] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]]];
                [imageView sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"Default_PostIcon"]];
                ...
                NSString * encodedString = [[aImageUrls objectAtIndex:i] stringByAddingPercentEncodingWithAllowedCharacters:set];
        ...
        _pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:83/255.0 green:186/255.0 blue:231/255.0 alpha:1.0];
